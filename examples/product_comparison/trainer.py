...
from auto_research.losses import make_loss_fn, IGNORE_INDEX
...
def train_one(
    ...
    If `collator` or `pos_token_id` are not supplied, they are derived from
    the tokenizer (ProductPairCollator + Yes-token id) so this trainer can
    also run standalone.
    """
    ...
    loss_fn = make_loss_fn(cfg.loss, pos_token_id=pos_token_id)
